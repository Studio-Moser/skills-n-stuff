...
It is a single file, so on REAL-FILE the diff is a plain `diff -u` (not `-r`); the target is
generated by Phase 1.5, so a machine-local edit found there was almost certainly a hand edit
of a file that will be regenerated — show the diff, then prefer the repo copy.
