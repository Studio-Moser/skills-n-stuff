...
assert parse_schema(request_schema) == [
    ...
    ("delegation", None),
    ("delegation.max_children", "positive integer"),
    ("delegation.max_depth", "positive integer"),
    ("delegation.token_budget", "positive bounded amount when the runtime supports it"),
    ("verification", None),
...
assert parse_schema(result_schema) == [
    ...
    ("route.executor", "current agent, native child, or external CLI"),
    ("route.dispatch", "direct | delegated"),
    ("artifacts", None),
