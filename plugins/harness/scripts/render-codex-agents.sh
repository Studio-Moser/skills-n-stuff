...
# --- compose ---------------------------------------------------------------
rendered="$(printf '%s\n\n%s\n\n%s\n%s\n\n%s\n\n%s\n\n%s\n' \
  '<!-- GENERATED by harness:sync (render-codex-agents.sh) from claude/output-styles/House Style.md and claude/CLAUDE.md — edit those; this file is overwritten on every sync. -->' \
  '# Rules' \
  "$rule_halluc" \
  "$rule_naming" \
  "$discipline" \
