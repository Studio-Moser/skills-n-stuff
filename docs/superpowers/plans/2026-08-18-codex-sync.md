...
- **Codex reads `~/.agents/skills` natively** (verified live in Task 4: Codex lists the store's skills with nothing under `~/.codex/skills`; `npx skills list -g --json` records Codex as an agent of those paths). No `-a codex` registration is needed; the comma form `-a claude-code,codex` is invalid (repeated `-a` is accepted but pointless). Phase 2.6 keeps `-a claude-code`. `--copy` is what produced today's stale copies — never pass it.
